...
<head>
    ...
    <style>
        ...
        .emoji-sidebar a, .emoji-sidebar form {
            transition: all 0.2s ease;
        }
        .emoji-sidebar a:hover, .emoji-sidebar form:hover button {
            transform: scale(1.1);
...
<div class="flex min-h-screen">
    ...
    <!-- ========== SIDEBAR KIRI (EMOJI ONLY) ========== -->
    <aside class="emoji-sidebar w-16 bg-white border-r border-gray-200 flex flex-col items-center py-6 space-y-6 shadow-sm">
        <!-- Menu -->
        <a href="{{ route('kasir.menus') }}" class="flex flex-col items-center justify-center w-10 h-10 rounded-xl text-gray-600 hover:bg-amber-50 hover:text-amber-500 transition-all duration-200" title="Menu">
            <span class="text-2xl">🍽️</span>
        </a>
        
        <!-- Kasir / Dashboard -->
        <a href="{{ route('kasir.dashboard') }}" class="flex flex-col items-center justify-center w-10 h-10 rounded-xl text-gray-600 hover:bg-amber-50 hover:text-amber-500 transition-all duration-200" title="Kasir">
            <span class="text-2xl">💰</span>
        </a>
        
        <!-- Riwayat Transaksi -->
        <a href="{{ route('kasir.riwayat') }}" class="flex flex-col items-center justify-center w-10 h-10 rounded-xl text-gray-600 hover:bg-amber-50 hover:text-amber-500 transition-all duration-200" title="Transaksi">
            <span class="text-2xl">📋</span>
        </a>
        
        <!-- Logout - FORM POST dengan konfirmasi -->
        <form method="POST" action="{{ route('kasir.logout') }}" class="mt-auto" id="logoutForm">
            @csrf
            <button type="button" 
                    onclick="if(confirm('Yakin ingin logout dari akun kasir?')) this.closest('form').submit();"
                    class="flex flex-col items-center justify-center w-10 h-10 rounded-xl text-gray-600 hover:bg-red-50 hover:text-red-500 transition-all duration-200" 
                    title="Logout">
                <span class="text-2xl">🚪</span>
            </button>
        </form>
    ...
    <!-- ========== SIDEBAR KANAN (BRAND + TIPE LAYANAN) ========== -->
    ...
    <aside class="w-full md:w-80 bg-white border-l border-gray-100 p-6 flex flex-col shadow-xl">
        ...
        <ul id="cart-items" class="flex-1 space-y-3 overflow-y-auto custom-scroll pr-2 hidden"></ul>
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ikon menu -> scroll ke menu container
